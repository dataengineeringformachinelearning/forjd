...
def _token_value(tokens: TokenMap, path: str) -> Any:
    ...
    for part in path.split("."):
        if not isinstance(current, dict) or part not in current:
            raise RuntimeError(f"Missing FJORD report token: {path}")
...
def _resolve_color(tokens: TokenMap, path: str) -> Color:
    ...
    while True:
        if current_path in visited:
            raise RuntimeError(f"Circular FJORD color token: {path}")
        visited.add(current_path)
        raw = _token_value(tokens, current_path)
        if not isinstance(raw, str):
            raise RuntimeError(f"FJORD color token is not a string: {current_path}")
        value = raw.strip()
...
def _resolve_points(tokens: TokenMap, path: str) -> float:
    ...
    if not isinstance(raw, str) or not raw.endswith("px"):
        raise RuntimeError(f"FJORD length token is not expressed in pixels: {path}")
...
def load_pdf_report_theme() -> PdfReportTheme:
    ...
    try:
        ...
        raise RuntimeError(
            "The FJORD token mirror (backend/static/fjord-report-tokens.json) is unavailable"
        ) from exc
...
def _append_brand_mark(
    ...
    # FORJD report mark translated from its 24 x 24 SVG geometry.
    commands.extend(
        [
...
def _page_streams(
    ...
    *,
    ...
    status_text = (
        f"Showing {shown:,} of {len(rows):,} records"
        if shown < len(rows)
        else f"{len(rows):,} records"
    )
