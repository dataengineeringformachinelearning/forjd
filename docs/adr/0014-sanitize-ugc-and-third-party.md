...
in partner UIs, but stored strings still reach APIs, PDFs, public status KPIs, and
...
1. **Central helpers** in `app/core/sanitize.py`:
   ...
   - `scrub_for_logs` — deep secret/ciphertext scrub for Sentry/Rollbar.
...
5. **Partner UIs** (e.g. DEML) own display escaping for any third-party strings
   they render.
